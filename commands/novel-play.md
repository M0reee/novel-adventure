...
argument-hint: "<world> <player action> [--slot slot]"
---
...
```bash
python novel.py play <world> "<player action>" [--slot <slot>]
```
...
- If the user names a save slot, pass `--slot <slot>` so state changes go to that save file.

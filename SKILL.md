...
Genre-combat rule: base math remains in `game_math.py`, but `combat.py` must load `combat_profiles.py` to add genre-specific risks. Xuanhuan/xianxia care about realm pressure and backlash; wuxia cares about wounds and reputation; cyberpunk/scifi care about alerts, tracking, gear, ammo, and energy; mystery cares about sanity and contamination; apocalypse cares about noise, infection, ammo, and morale. These consequences should be structured state changes, not pure narration.
